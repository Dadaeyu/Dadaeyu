 ...
 *   node scripts/apply-auth-phase.mjs all  # phase1 + phase2 + phase3 + phase4
 *
...
async function main() {
  ...
  const phases =
    arg === "all" ? [1, 2, 3, 4] : [Number(arg)].filter((p) => PHASE_FILES[p]);
  ...
  if (env.SUPABASE_ACCESS_TOKEN) {
    ...
    if (phases.includes(4)) {
      console.log(
        "\n이메일 인증 가입: Supabase 대시보드에서 Confirm email ON, Redirect URL(/auth/callback) 등록을 확인하세요."
      );
    }
    return;
  }
