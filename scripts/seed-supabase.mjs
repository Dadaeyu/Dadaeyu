...
const PLACES = [
  ...
  },
  {
    ...
    hot: true,
    description:
      "1956년 창업한 대전 최고의 빵집으로, 튀김소보로와 판타롱 부추빵 등으로 유명합니다.",
  ...
  },
  {
    ...
    hot: false,
    description:
      "충남·세종·충북 지역에서 가장 유명한 온천 지역으로, 알칼리성 중탄산나트륨 온천수가 특징입니다.",
...
// 기존 리뷰 삭제 후 재삽입
await supabase.from("tb_place_reviews").delete().neq("id", 0);
const { error: reviewsError } = await supabase.from("tb_place_reviews").insert(REVIEWS);
if (reviewsError) {
