                  ...
                  <div className="border border-gray-200 rounded-xl overflow-hidden mt-1">
                    ...
                    {PLACES.filter(p => !(editDays.find(d => d.day === activeDay)?.places ?? []).some(ep => ep.name === p.name)).map(p => (
                      <button key={p.id} onClick={() => {
                        ...
                        <div className="flex items-center gap-2">
                          <span className="text-xs px-1.5 py-0.5 rounded-full font-medium" style={{ background: PLACE_COLORS[p.colorKey].bg, color: PLACE_COLORS[p.colorKey].color }}>{p.category}</span>
