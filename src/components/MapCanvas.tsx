...
export function MapCanvas({ className = "w-full h-full", routePlaces = [], onClick, onPinClick }: Props) {
  return (
    <svg
    ...
    >
      ...
      {/* 갑천 */}
      <path d="M 152 0 C 144 110,170 195,160 305 C 150 390,128 445,150 535 C 162 582,156 642,146 700"
        stroke="#aedcf8" strokeWidth="26" fill="none" strokeLinecap="round" />
      {/* 유등천 */}
      <path d="M 0 458 C 52 442,104 462,150 452"
        stroke="#aedcf8" strokeWidth="18" fill="none" strokeLinecap="round" />
      ...
      {routePlaces.length > 1 && routePlaces.map((place, i) => {
        if (i === routePlaces.length - 1) return null;
        const next = routePlaces[i + 1];
        return (
          <line
            key={`route-line-${i}`}
            x1={place.cx} y1={place.cy - 14}
            x2={next.cx} y2={next.cy - 14}
            stroke="#16a34a"
            strokeWidth="4"
      ...
      {routePlaces.map((place, i) => {
        ...
        return (
          ...
          >
            ...
            <polygon points={`${cx - 8},${cy - 4} ${cx + 8},${cy - 4} ${cx},${cy + 8}`} fill={color} />
            {/* White ring */}
