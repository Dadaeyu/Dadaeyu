      ...
      <div className={cn("flex gap-2 overflow-x-auto", className)}>
        {items.map(({ key, label, count }) => (
          <button
            ...
            className={cn(
              "min-h-11 rounded-full px-4 py-2 text-sm font-medium whitespace-nowrap transition-colors",
    ...
    <div className={cn("border-hairline flex overflow-x-auto border-b", className)}>
      {items.map(({ key, label, count }) => (
        <button
          ...
          onClick={() => onValueChange(key)}
          className={cn(
            "flex min-h-11 items-center gap-1.5 px-4 py-2.5 text-sm font-semibold whitespace-nowrap transition-colors",
            value === key
