...
const ACCESSIBILITY_COLS: Record<string, string[]> = {
  "보행":   ["has_parking", "has_route", "has_public_transport", "has_ticket_office", "has_wheelchair", "has_exit", "has_elevator", "has_restroom", "has_room"],
...
export async function GET(request: Request) {
  ...
  const accessTypes = (searchParams.get("accessibility") ?? "")
    .split(",")
    .filter(Boolean);

  if (!keyword.trim() && accessTypes.length === 0) return Response.json([]);
