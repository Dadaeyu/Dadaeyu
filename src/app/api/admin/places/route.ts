...
async function upsertPlaceExtras(
  ...
  const { error } = await supabase.from("tb_place_barrierfree").upsert(row, { onConflict: "place_id" });
  if (error) throw error;
}
...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (idParam) {
      ...
      const [{ data: place, error: placeError }, { data: detail }, { data: bf }] = await Promise.all([
        supabase.from("tb_place").select(LIST_COLUMNS).eq("place_id", placeId).ilike("contentid", "a%").maybeSingle(),
        supabase
