...
async function buildAdminCourses(
  ...
  const themeCodes = [
    ...new Set([...placeMap.values()].map((p) => p.lclssystm1).filter((v): v is string => v != null))
  ];
...
export async function PATCH(request: Request) {
  ...
  if (!Number.isFinite(courseId)) return NextResponse.json({ error: "Invalid id" }, { status: 400 });
  ...
  try {
    ...
    const { error } = await supabase
      .from("tb_course")
      .update({ delete_yn: "N", deletetime: null, deleter: null, updatetime: new Date().toISOString() })
