...
function mapMemberUpdateError(error: { code?: string; message: string }): Error {
  if (error.code === "23505" || error.message.includes("members_nickname_unique")) {
...
export async function updateMember(
  ...
  patch: Partial<
    Pick<
      DbMember,
      "nickname" | "phone" | "avatar_url" | "gender" | "age_group" | "onboarding_completed"
    >
  >
) {
  const supabase = createClient();
...
export async function updateUserPreferences(
  ...
  if (error) throw error;
  if (data) return data as DbUserPreferences;
  ...
  return inserted as DbUserPreferences;
}
