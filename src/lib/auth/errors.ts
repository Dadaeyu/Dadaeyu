...
export function mapAuthError(error: {
  ...
  if (code === "resend_cooldown") {
    const retryMs = error.retryAfterMs ?? 5000;
    const sec = Math.max(1, Math.ceil(retryMs / 1000));
    return `인증 메일은 ${sec}초 후에 다시 보낼 수 있습니다.`;
  }

  if (code === "over_email_send_rate_limit") {
    return "Supabase 메일 발송 한도에 도달했습니다. 메일함에 이전 인증 메일이 있는지 확인하거나, 잠시 후 다시 시도해 주세요.";
  }
  ...
  if (msg.includes("Error sending confirmation email")) {
    return "인증 메일을 보내지 못했습니다. Supabase SMTP 설정을 확인해 주세요. (개발 서버에서는 가입 후 테스트용 링크가 표시될 수 있습니다.)";
  }
